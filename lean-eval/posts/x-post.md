...
The core lesson is that flow and agent loops can amplify model capability. Before the loop starts, we ask a model to turn the mathematical idea into an explicit natural-language proof. The worker then maps that proof to the exact Lean statement, Mathlib APIs, and bridge lemmas.

The flow and the review loop keep the goal fixed across rounds. One agent implements, an independent reviewer finds gaps, and compiler errors, failed verification, and review feedback become work for the next round. The Stop hook prevents premature completion. The loop does not replace reasoning; it gives the model structure, memory, external feedback, and repeated chances to recover from mistakes.
