...
Notably, the same Codex harness also runs **open source models like Kimi-K3** on these tasks. The plans and problem-only worker seeds are model-independent; reproducing a run with Kimi only requires changing the Codex API key and model name. As widely shared, we all love _open models X open harness_ 🎉 and the combination achieves full score at every competiton: 
...
```sh
git clone https://github.com/anonymous/ioi2026.git
cd ioi2026
./verify.sh
```
