#!/usr/bin/env python3
"""Native persisted Kimi-K3 goals, a hardware vendor Responses with ordering relay; no answer reuse."""
import asyncio
import argparse
import json
import os
...
BASE=Path('/home/user/icho-native-goal-kimi68-20260914-01')
MODEL='vendor/moonshotai/kimi-k3'
native.BASE=BASE
native.MODEL=MODEL
ORIGINAL_COMMAND=native.command
PROVIDER_URL='https://inference-api.vendor.example/v1'
...
def command(args,argv):
    ...
    i=cmd.index('/home/user/.codex/auth.json')
...
def provider_config():
    return ['model='+json.dumps(MODEL),'model_provider="vendor_icho"',
        'model_providers.vendor_icho.name="Vendor IChO Kimi"',
        'model_providers.vendor_icho.base_url='+json.dumps(PROVIDER_URL),
        'model_providers.vendor_icho.env_key="ICHO_KIMI_API_KEY"',
        'model_providers.vendor_icho.wire_api="responses"',
        'model_providers.vendor_icho.requires_openai_auth=false',
        'model_providers.vendor_icho.supports_websockets=false',
        'model_providers.vendor_icho.stream_idle_timeout_ms=600000',
        'model_providers.vendor_icho.request_max_retries=8',
...
def prepare():
    ...
    pdfcache=Path('/home/user/.cache/uv/archive-v0/wDOgr5V0cDEN5-3z')
    for name in ('pymupdf','fitz'):
        shutil.copytree(pdfcache/name,BASE/'runtime/src'/name)
    m=json.loads((BASE/'manifest.json').read_text())
    m.update(provider='vendor_icho',base_url='https://inference-api.vendor.example/v1',
        ...
        credential_policy='environment-only a hardware vendor key; no OpenAI auth mount; never exported',
...
def configure_catalog():
    ...
    cache=json.loads(Path('/home/user/.codex/models_cache.json').read_text())
    ...
    model.update(slug=MODEL,display_name='Kimi-K3 (a hardware vendor)',description='User-selected Kimi-K3 through a hardware vendor Responses',
