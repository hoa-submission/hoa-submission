# IChO 2026 review-loop experiment: Kimi-K3 max
...
reasoning for both implementation and every harness review.
...
The prior experiment used the harness's native Stop-hook state machine. This run
uses the harness's `ask-codex` consultation workflow and records its own explicit
round ledger; it does not claim native loop state files that it did not create.
...
Each candidate is then sent to Kimi-K3 through the harness's `ask-codex`
...
a separate Kimi-K3/max audit both had to end in `COMPLETE`. The prior
...
The original text-only harness loops all reached `COMPLETE`, but that was not
...
implementation/review round; “focused” is the final image-aware plus harness
audit round.

| Problem | Preserved source branch | Original | Focused | Final source head | Bytes | Result |
|---|---|---:|---:|---:|---:|---|
| Q1 | `worker-kimi-k3max-q01` | 4 | 1 | `f3405ff2` | 10092 | Corrected; COMPLETE |
| Q2 | `worker-kimi-k3max-q02` | 2 | 0 | `b28054c9` | 9238 | Unchanged; COMPLETE |
| Q3 | `worker-kimi-k3max-q03` | 3 | 1 | `e9a988e3` | 16716 | Corrected; COMPLETE |
| Q4 | `worker-kimi-k3max-q04` | 0 | 0 | `b8a900fc` | 7270 | Corrected; COMPLETE |
| Q5 | `worker-kimi-k3max-q05` | 1 | 1 | `b8d8658c` | 16764 | Corrected; COMPLETE |
| Q6 | `worker-kimi-k3max-q06` | 0 | 3 | `1b859e6f` | 18671 | Corrected; COMPLETE |
| Q7 | `worker-kimi-k3max-q07` | 1 | 0 | `e9a600f7` | 12018 | Corrected; COMPLETE |
| Q8 | `worker-kimi-k3max-q08` | 0 | 0 | `85bb1b47` | 13053 | Corrected; COMPLETE |
| Q9 | `worker-kimi-k3max-q09` | 2 | 1 + final 0 | `ed504bb4` | 18145 | Corrected twice; COMPLETE |
...
- all nine original `runtime/status.json` ledgers have `state=complete`,
  `ok=true`, `harness_complete=true`, validator exit 0, `model=Kimi-K3`,
...
- all nine final post-audit ledgers have both `image_audit_complete=true` and
  `harness_complete=true`, and both final review files end in `COMPLETE`;
- all 48 retained harness metadata records report `model: Kimi-K3`,
...
and harness audit all agree on those values.
...
- Consolidated worktree:
  `/root/user-workspace/icho-2026-kimi-k3-max-results`
